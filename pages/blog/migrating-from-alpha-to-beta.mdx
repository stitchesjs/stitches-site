...
+ import { css, styled, global, theme, keyframes, toString } = createCss()
```
...
- The `getStyle` function has been renamed to `toString`.
- The `toString` function receives no arguments, just call it and it'll render your styles as a string.
...
#### Choose your default variants

You can now define default variants in `css` and `styled` functions via a `defaultVariants` key in the style object.
...
#### Choose your property to theme mapping

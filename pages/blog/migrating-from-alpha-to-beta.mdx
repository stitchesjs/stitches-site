...
#### Only one ampersand per selector

You can only use one ampersand (`&`) per selector.
If you were using two ampersands (`&&`) to increase the specificity of a rule, check that it's still necessary, as we've fixed lots of specificity issues.
...
If for some reason you need to increase specificity, we recommend adding the class attribute selector:
...
You can now pick a token from a specific scale instead of the mapped one.
For example, while `marginTop` is mapped to the `space` scale, now it's possible to pick a token from the `size` scale by prefixing it with the scale name.

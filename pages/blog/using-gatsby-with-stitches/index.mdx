...
<Image title="Hero image of Gatsby and Stitches" src={hero} size="hero" />
...
First and foremost, istall the Gatsby CLI:
...
export { styled, css } = createStyled({
  tokens: {
    fonts: {
      $system: 'system-ui',
    },
...
exports.replaceRenderer = ({ bodyComponent, setHeadComponents }) => {
  ...
  setHeadComponents(
    styles.map((sheet, i) =>
      createElement('style', { key: i, dangerouslySetInnerHTML: { __html: sheet } }, null)
    )
  );
};
```
